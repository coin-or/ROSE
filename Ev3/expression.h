...
public:
  ...
  Operand(const double c, const Int t, std::string vn);
  ...
  // get the lower bound
  double GetLB(void) const;
  
  // get the upper bound
  double GetUB(void) const;
...
public:
  ...
  // create a variable leaf and set coefficient
  BasicExpression(const double c, const Int t, std::string vn);
  ...
  // number of levels in an expression tree
  int GetNumberOfLevels(void);
  void GetNumberOfLevelsRecursive(int& maxlevel);

  // number of explicit levels in an expression tree
  // (i.e. coeff*node^expon becomes a subtree of 4 nodes and so on)
  int GetNumberOfExplicitLevels(void);
  void GetNumberOfExplicitLevelsRecursive(int& maxlevel);
  
  // get number of distinct constants in an expression tree
  int GetNumberOfDistinctConstants(void);
  void GetNumberOfDistinctConstantsRecursive
    (std::map<double,int,DoubleLessThan>& ConstOccurrence);
  ...
  // output is a tree
  std::string PrintTree(int blanks, int tabs) const;
  ...
  // output the tree as a list of arcs
  // returns number n of nodes in tree (numbered 1..n)
  // [in]  r: index of the root node
  // [out] edgeList: list of edges
  // [out] nodeType: the node types 
  // [out] index: maps nodes to variable indices (0 if not a var node)
  // [out] coeff: maps nodes to constant variables (0 if not a var node)
  int TreeData(int r, std::vector<std::pair<int,int> >& edgeList, 
	       std::map<int,int>& nodeType, std::map<int,int>& index, 
	       std::map<int,double>& coeff,
	       std::map<int,std::pair<double,double> >& bnds);
  void TreeRecursive(int r, int& maxr, 
  ...
  double FastEval(double* varvalues, std::map<int,int>& varmap, Int vsize);  
  ...
  void ReplaceVariable(int v1, int v2, std::string vn);
  void ReplaceVariable(int v1, int v2, std::string vn, double c2);
  ...
  Expression ReplaceBySchema(int vi, std::string vn, Expression schema);
  // works on subnodes not on current node
  Expression ReplaceBySchemaRecursive(int vi, std::string vn,Expression schema);

  // replace with a variable the deepest node with given operator label
  // return replaced term or zero expression if no replacement occurs
  Expression ReplaceByOperator(int vi, std::string vn, int oplabel);  
  // works on subnodes not on current node
  Expression ReplaceByOperatorRecursive(int vi, std::string vn, int oplabel);  
  ...
  // smith's standard form: takes a starting added variable index,
  // the index of the first added variable, a variable
  // name, a vector of oplabels and one of schemata; performs replacements
  ...
  void SmithStandardForm(int defvarindex, int addedvarstartindex, 
			 std::string defvarname,
			 std::vector<int>& oplabels, std::vector<Expression>& schemata,
			 std::vector<Expression>& defcons);
  void SmithStandardFormRecursive(int defvarindex, 
				  int addedvarstartindex, 
				  std::string defvarname,
				  std::vector<int>& oplabels, 
				  std::vector<Expression>& schemata,
				  std::vector<Expression>& defcons);
  // auxiliary function to SmithStandardFormRecursive()
  // finds *this in defcons
  int SmithFindInDefcons(int addedvarstartindex, std::vector<Expression>& defcons);
  ...
  int ProdBinCont(int defvarindex, std::string defvarname,
		  std::map<int,bool>& integrality,
		  std::map<int,double>& vlb, 
		  std::map<int,double>& vub,
		  std::map<int,std::string>& vstr,
		  std::map<int,std::pair<double,double> >& addvarbounds,
		  std::vector<Expression>& defcons);
  // returns number of replacements
  int ProdBinContRecursive(int defvarindex, std::string defvarname, 
			   std::map<int,bool>& integrality,
			   std::map<int,double>& vlb, 
			   std::map<int,double>& vub,
			   std::map<int,std::string>& vstr,
  ...
  void ResetVarNames(std::string vn, int lid, int uid);
  ...
  void GetVarIndicesInSchema(std::vector<int>& vidx, Expression schema);
  ...
  void Interval(std::map<int,double> Vlb, std::map<int,double> Vub, 
		double& elb, double& eub);

  // feasibility-based bounds tightening: performs one up/down iteration
  // Vlb/Vub input lower/upper bounds for variables, and updated for
  // output; elb/eub: constraint bounds (input only)
  void FBBTUpDown(std::map<int,double>& Vlb, std::map<int,double>& Vub, 
		  double elb, double eub);
  void FBBTDown(std::map<int,double>& Vlb, std::map<int,double>& Vub, 
		double elb, double eub);
  ...
  // nodes are given node ID startnode + topnode, and color
  // level*totalops + optype
  // DAG: the existing DAG - expression trees are added to this DAG
  // VOp: operator label of each vertex
  // VColor: the final color map for isolating possible symmetries
  // ExprSymm: map eidx->symmclass
  // eidx: current expression index
  // maxcolor: upper bound on number of colors in each expr (over all exprs)
  // startnode: coding of nodes starts here (usually max VarID in problem)
  // topnode: the node id assigned to the root node of the (sub)expr
  // totalops: the total number of operator labels in ROSE
  // level: the level counter (start from 1, used internally)
  void AddExpressionTreeToDAG(std::map<int,std::vector<int> >& DAG, 
			      std::map<int,int>& VOp, 
			      std::map<int,std::set<int> >& VColor,
			      std::map<int,int>& ExprSymm,
			      int eidx,
			      std::map<double,int,DoubleLessThan>& ConstantSymm,
			      int varsymmclasses, 
			      std::map<int,int>& VarSymm,
			      int maxcolor, 
			      int startnode, int& topnode, 
			      int totalops, int level);
