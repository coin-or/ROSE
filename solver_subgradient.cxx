/*
** Name:       solver_Subgradient.cxx
** Author:     Lucas Letocart
** Source:     GNU C++
** Purpose:    Solver Subgradient implementation
...
SubgradientSolver::SubgradientSolver() {
  TheName = "subgradient";
  NumberOfVariables = 0;
...
void SubgradientSolver::SetProblem(Problem* p) {
  InProb = p; 
  InitProb = false;
}

bool SubgradientSolver::CanSolve(int problemtype) {
  // fill in here with information about solver capabilities
}

void SubgradientSolver::Initialize(bool force = false) {
  ...
  if (InProb == NULL) {
    cerr << "SubgradientSolver::Initialize(): error: InProb is NULL\n";
    assert(InProb != NULL);
  }
  ...
  for(int i = 1; i <= np; i++) {
    if (ParameterBlob.GetParameterName(i) == "SubgradientLocalSolver") {
      LocalSolverName = ParameterBlob.GetParameterStringValue(i);
    } else if (ParameterBlob.GetParameterName(i) == "SubgradientMaxTime") {
      MaxRunningTime = (double) ParameterBlob.GetParameterIntValue(i);
      assert(MaxRunningTime >= 0);
    } else if (ParameterBlob.GetParameterName(i) == "SubgradientEpsilon") {
      Epsilon = ParameterBlob.GetParameterDoubleValue(i);
      assert(Epsilon > 0);
    } else if (ParameterBlob.GetParameterName(i) == "SubgradientQuiet" ||
...
int SubgradientSolver::Solve(bool reinitialize) {
  ...
  if (!Quiet) {
    cout << "SubgradientSolver: best solution f* = " << OptimalObjVal << endl;
  }
  ...
  if (IsFeasible) {
    ...
    if (!Quiet) {
      cout << "SubgradientSolver: this solution is infeasible by at least " 
           << discrepancy << endl;
    }
    InProb->SetFeasible(-1);
  }
...
void SubgradientSolver::SetOptimizationDirection(int theoptdir) {
...
void SubgradientSolver::GetSolution(map<int,double>& objfunval, 
...
void SubgradientSolver::GetSolutionLI(vector<double>& objfunval, 
...
void SubgradientSolver::GetSolutionLI(double& objfunval, 
				   vector<double>& solution) {
  if (IsSolved) {
...
void SubgradientSolver::SetStartingPoint(int varID, double sp) {
  int localindex = InProb->GetVarLocalIndex(varID);
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  x[localindex - 1] = sp;
}

void SubgradientSolver::SetStartingPointLI(int localindex, double sp) {
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  x[localindex - 1] = sp;
}

double SubgradientSolver::GetStartingPoint(int varID) const {
  int localindex = InProb->GetVarLocalIndex(varID);
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  return x[localindex - 1];
}

double SubgradientSolver::GetStartingPointLI(int localindex) const {
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  return x[localindex - 1];
}

void SubgradientSolver::SetVariableLB(int varID, double LB) {
  int localindex = InProb->GetVarLocalIndex(varID);
  SetVariableLBLI(localindex, LB);
}

double SubgradientSolver::GetVariableLB(int varID) const {
  int localindex = InProb->GetVarLocalIndex(varID);
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  return vlb[localindex - 1];
}

void SubgradientSolver::SetVariableUB(int varID, double UB) {
  int localindex = InProb->GetVarLocalIndex(varID);
  SetVariableUBLI(localindex, UB);
}

double SubgradientSolver::GetVariableUB(int varID) const {
  int localindex = InProb->GetVarLocalIndex(varID);
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  return vub[localindex - 1];
}

void SubgradientSolver::SetVariableLBLI(int localindex, double LB) {
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  vlb[localindex - 1] = LB;
}

double SubgradientSolver::GetVariableLBLI(int localindex) const {
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  return vlb[localindex - 1];
}

void SubgradientSolver::SetVariableUBLI(int localindex, double UB) {
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  vub[localindex - 1] = UB;
}

double SubgradientSolver::GetVariableUBLI(int localindex) const {
  assert(localindex >= 1 && localindex <= NumberOfVariables);
  return vub[localindex - 1];
}
